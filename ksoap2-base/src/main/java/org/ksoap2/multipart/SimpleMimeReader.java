 ...
 *      Made boundary calls work with Collabnet Teamforge response types  as the boundary doesnt come as part of content-type header,
 *      Check the getBoundary(String header) for a sample response
 *      Changed package to accomodate compile
 */
...
public class SimpleMimeReader
{
    ...
    public SimpleMimeReader (InputStream inStream)
    {
        ...
        this.in = new BufferedInputStream(inStream, 100000);
        getMimeBoundary();
    }
    ...
    /**
     * Get the Boundary line from the header. In CTF's case, the boundary doesnt appear in the
     * Content-type header but something like this
     *
     * --<boundary>
     * <headers>
     *
    ...
    private void getMimeBoundary ()
    {
        ...
        boundary = getBoundary(docHeader);

        /**
         * Raja -- Commented in favour of CTF's Mime Soap message
